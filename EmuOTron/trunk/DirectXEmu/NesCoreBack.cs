...
namespace DirectXEmu
{
    class NESCore
    {
        ...
        private bool isStateStored = false;
        ...
        private byte[] PPUMemory = new byte[0x8000];
        ...
        public int patternTablesPalette = 0;
        ...
        public void Start()
        {
            ...
            while (this.emulationRunning)
            {
                ...
                #endregion
                if (this.mapper == 4 && this.vblank == 0)
                {
                    if (this.MMC3.irqEnable)
                    {
                        if (this.MMC3.irqReload)
                        {
                            this.MMC3.irqCounter = this.MMC3.irqLatch;
                            this.MMC3.irqReload = false;
                        }
                        this.MMC3.irqCounter--;
                        if (this.MMC3.irqCounter == 0)
                            this.interruptIRQ = true;
                    }
                }
                ...
                else if ((this.interruptIRQ || this.apu.frameIRQ) && (this.P & 0x04) == 0)
                {
                    this.pushWordStack(this.programCounter);
                    this.pushByteStack(this.P);
                    this.P |= 0x04;
                ...
                if (this.counter >= this.scanlineLengths[this.slCounter])
                {
                    ...
                    if (this.scanline >= 0 && this.scanline < 240)
                    {
                        if (!turbo)
                            ...
                            this.spriteZeroHit = SpriteZeroHit(this.scanline, 114);
                    }
                    ...
                    if (this.generatePatternTables && this.scanline == this.generatePatternLine)
                        this.patternTables = this.GeneratePatternTables(this.patternTablesPalette);
        ...
        private void VerticalReset() { }
        private void HorizontalReset() { }
        public NESCore(String input)
        {
            this.Memory = new MemoryStore(0x40, false);
            ...
            //this.interruptReset = true; //Made this change when I added program counter setter at end of construct to prevent 1 incorrect opcode being execute at power up before interrupt is processed
            this.filePath = input;
            ...
            this.isStateStored = false;
            int numprgrom = inputStream.ReadByte();
            int numvrom = inputStream.ReadByte();
            int lowMapper = inputStream.ReadByte();
            bool trainer = ((lowMapper & 0x04) != 0) ? true : false;
            bool vertMirroring = ((lowMapper & 0x01) != 0) ? true : false;
            this.sramPresent = ((lowMapper & 0x02) != 0) ? true : false;
            int highMapper = inputStream.ReadByte();
            inputStream.Position = 0x0F;
            if (inputStream.ReadByte() != 0)
                highMapper = 0;
            bool PC10 = ((highMapper & 0x02) != 0) ? true : false;
            bool VS = ((highMapper & 0x01) != 0) ? true : false;
            mapper = (lowMapper >> 4) + (highMapper & 0xF0);
            this.Memory = new MemoryStore(0x40 + (numprgrom * 0x10), false);
            ...
            romInfo.AppendLine("Mirroring: " + (vertMirroring ? "Vertical" : "Horizontal"));
            if (VS)
            ...
            this.PRGRom = new byte[numprgrom][];
            for (int i = 0x00; i < numprgrom; i++)
            {
                this.PRGRom[i] = new byte[0x4000];
                for (int j = 0x00; j < 0x4000; j++)
                {
                    this.PRGRom[i][j] = (byte)inputStream.ReadByte();
                }
            }
            for (int i = 0; i < PRGRom.Length; i++)
            {
                for (int j = 0x0000; j < 0x4000; j++)
                {
                    Memory.banks[Memory.swapOffset + (j / 0x400) + ((i) * 0x10)][j % 0x400] = PRGRom[i][j];
                }
            }
            this.CHRRom = new byte[numvrom][];
            for (int i = 0x00; i < numvrom; i++)
            {
                this.CHRRom[i] = new byte[0x2000];
                for (int j = 0x00; j < 0x2000; j++)
                {
                    this.CHRRom[i][j] = (byte)inputStream.ReadByte();
                }
            }
            if (PC10)
            {
            ...
            switch (mapper)
            {
                case 0://NROM
                    ...
                    Memory.Swap16kROM(0xC000, PRGRom.Length - 1);/*
                    if (this.PRGRom.Length == 1)
                    {
                        for (int i = 0x0000; i < 0x4000; i++)
                        {
                            this.Memory[i + 0x8000] = this.PRGRom[0][i];
                            this.Memory[i + 0xC000] = this.Memory[i + 0x8000];
                            this.readOnly[i + 0x8000] = true;
                            this.readOnly[i + 0xC000] = true;
                        }
                    }
                    else if (this.PRGRom.Length == 2)
                    {
                        for (int i = 0x0000; i < 0x4000; i++)
                        {
                            this.Memory[i + 0x8000] = this.PRGRom[0][i];
                        ...
                        for (int i = 0x0000; i < 0x4000; i++)
                        {
                            this.Memory[i + 0xC000] = this.PRGRom[1][i];
                            this.readOnly[i + 0xC000] = true;
                        }
                    }*/
                    if (this.CHRRom.Length == 1)
                    {
                        ...
                        {
                            this.PPUMemory[i] = this.CHRRom[0][i];
                            this.PPUReadOnly[i] = true;
                        }
                    }
                    break;
                case 1: //MMC1
                    ...
                    for (int i = 0x0000; i < 0x4000; i++)
                    {
                        this.Memory[i + 0xC000] = this.PRGRom[this.PRGRom.Length - 1][i];
                        this.readOnly[i + 0xC000] = true;
                    }
                    this.MMC1.highCHRBank = 1;
                    this.MMC1.lowCHRBank = 0;
                    if (this.CHRRom.Length == 0)
                    {
                        this.MMC1.chrBanks = new byte[0xFF][];
                        for (int i = 0; i < 0xFF; i++)
                            this.MMC1.chrBanks[i] = new byte[0x1000]; // barf
                    }
                    else
                    {
                        this.MMC1.chrBanks = new byte[this.CHRRom.Length * 2][];
                        for (int i = 0; i < this.CHRRom.Length; i++)
                        {
                            this.MMC1.chrBanks[(i * 2)] = new byte[0x1000];
                            this.MMC1.chrBanks[(i * 2) + 1] = new byte[0x1000];
                            for (int j = 0x00; j < 0x1000; j++)
                            {
                                this.MMC1.chrBanks[(i * 2)][j] = this.CHRRom[i][j];
                            }
                            for (int j = 0x00; j < 0x1000; j++)
                            {
                                this.MMC1.chrBanks[(i * 2) + 1][j] = this.CHRRom[i][j + 0x1000];
                            }
                        }
                        for (int i = 0x0000; i < 0x2000; i++)
                        {
                ...
                case 3://CNROM
                    ...
                    break;
                case 4: //MMC3
                    this.MMC3.prgBanks = new byte[this.PRGRom.Length * 2][];
                    for (int i = 0; i < this.PRGRom.Length; i++)
                    {
                        for (int j = 0; j < 2; j++)
                        {
                            this.MMC3.prgBanks[(i * 2) + j] = new byte[0x2000];
                            for (int k = 0x00; k < 0x2000; k++)
                            {
                                this.MMC3.prgBanks[(i * 2) + j][k] = this.PRGRom[i][k + (j * 0x2000)];
                            }
                    ...
                    for (int i = 0x0000; i < 0x2000; i++)
                    {
                        this.Memory[i + 0x8000] = this.MMC3.prgBanks[0][i];
                        this.readOnly[i + 0x8000] = true;
                    }
                    for (int i = 0x0000; i < 0x2000; i++)
                    {
                        this.Memory[i + 0xA000] = this.MMC3.prgBanks[1][i];
                        this.readOnly[i + 0xA000] = true;
                    }
                    for (int i = 0x0000; i < 0x2000; i++)
                    {
                        this.Memory[i + 0xC000] = this.MMC3.prgBanks[this.PRGRom.Length - 2][i];
                        this.readOnly[i + 0xC000] = true;
                    }
                    for (int i = 0x0000; i < 0x2000; i++)
                    {
                        this.Memory[i + 0xE000] = this.MMC3.prgBanks[this.PRGRom.Length - 1][i];
                        this.readOnly[i + 0xE000] = true;
                    }

                    this.MMC3.chrBank1 = 0;
                    this.MMC3.chrBank2 = 1;
                    this.MMC3.chrBank3 = 2;
                    this.MMC3.chrBank4 = 3;
                    if (this.CHRRom.Length == 0)
                    {
                        this.MMC3.chrBanks = new byte[0xFF][];
                        for (int i = 0; i < 0xFF; i++)
                            this.MMC3.chrBanks[i] = new byte[0x400]; // barf
                    }
                    else
                    {
                        this.MMC3.chrBanks = new byte[this.CHRRom.Length * 8][];
                        for (int i = 0; i < this.CHRRom.Length; i++)
                        {
                            for (int j = 0; j < 8; j++)
                            {
                                this.MMC3.chrBanks[(i * 8) + j] = new byte[0x400];
                                for (int k = 0x00; k < 0x400; k++)
                                {
                                    this.MMC3.chrBanks[(i * 2) + j][k] = this.CHRRom[i][k + (j * 0x400)];
                                }
            ...
            {
                //Vert Mirrioring
                this.PPUMirror(0x2000, 0x2800, 0x0400, 1);
                this.PPUMirror(0x2400, 0x2C00, 0x0400, 1);
            }
            else
            {
                //Horz Mirrioring
                this.PPUMirror(0x2000, 0x2400, 0x0400, 1);
                this.PPUMirror(0x2800, 0x2C00, 0x0400, 1);
            }
        ...
        private void writeByte(ushort address, byte value)
        {
            ...
            if (this.mapper == 1)
            {
                if (this.MirrorMap[address] >= 0x8000)
                {
                    if ((value & 0x80) != 0)
                    {
                        this.MMC1.writeLatch = 0;
                        this.MMC1.reg0 |= 0x0C;
                        for (int i = 0x0000; i < 0x4000; i++)//following lines may be wrong, based on nesdev wiki on mmc1
                        {
                            this.Memory[i + 0xC000] = this.PRGRom[this.PRGRom.Length - 1][i];
                            this.readOnly[i + 0xC000] = true;
                        }
                    }
                    else if (this.MMC1.writeLatch != 4)
                    {
                        this.MMC1.regTmp += (byte)((value & 1) << this.MMC1.writeLatch);
                        this.MMC1.writeLatch++;
                    }
                    else if (this.MirrorMap[address] >= 0xE000) //Prg reg  
                    {
                        this.MMC1.regTmp += (byte)((value & 1) << this.MMC1.writeLatch);
                        this.MMC1.reg3 = this.MMC1.regTmp;
                        this.MMC1.regTmp = 0;
                        this.MMC1.writeLatch = 0;
                        if ((this.MMC1.reg0 & 0x8) != 0) // Switch 16kb
                        {
                            if ((this.MMC1.reg0 & 0x4) != 0) // Switch at $8000
                            {
                                if ((this.MMC1.reg3) < this.PRGRom.Length)
                                {
                                    for (int i = 0x0000; i < 0x4000; i++)
                                    {
                                        this.Memory[i + 0x8000] = this.PRGRom[this.MMC1.reg3][i];
                                    }
                                }
                                for (int i = 0x0000; i < 0x4000; i++)
                                {
                            ...
                            else // Switch at $c000
                            {
                                ...
                                if ((this.MMC1.reg3) < this.PRGRom.Length)
                                {
                                    for (int i = 0x0000; i < 0x4000; i++)
                                    {
                                        this.Memory[i + 0xC000] = this.PRGRom[this.MMC1.reg3][i];
                                    }
                                }
                            }
                        }
                        ...
                        {
                            if ((this.MMC1.reg3 & 0xFE) < this.PRGRom.Length)
                            {
                                for (int i = 0x0000; i < 0x4000; i++)
                                {
                                    this.Memory[i + 0x8000] = this.PRGRom[this.MMC1.reg3 & 0xFE][i];
                                }
                                for (int i = 0x0000; i < 0x4000; i++)
                                {
                                    this.Memory[i + 0xC000] = this.PRGRom[(this.MMC1.reg3 & 0xFE) + 1][i];
                                }
                            }
                        }

                    }
                    else if (this.MirrorMap[address] >= 0xC000) // Chr Reg 1
                    {
                        this.MMC1.regTmp += (byte)((value & 1) << this.MMC1.writeLatch);
                        this.MMC1.reg2 = this.MMC1.regTmp;
                        this.MMC1.regTmp = 0;
                        this.MMC1.writeLatch = 0;
                        if ((this.MMC1.reg0 & 0x10) != 0)
                        {
                            if (this.CHRRom.Length == 0)
                            {
                                for (int i = 0x0000; i < 0x1000; i++)
                                {
                                    this.MMC1.chrBanks[this.MMC1.highCHRBank][i] = this.PPUMemory[i + 0x1000];
                                }
                                this.MMC1.highCHRBank = this.MMC1.reg2;
                                for (int i = 0x0000; i < 0x1000; i++)
                                {
                                    this.PPUMemory[i + 0x1000] = this.MMC1.chrBanks[this.MMC1.highCHRBank][i];
                                }
                            }
                            else
                            {
                                this.MMC1.highCHRBank = this.MMC1.reg2;
                                if (this.MMC1.highCHRBank < this.MMC1.chrBanks.Length)
                                {
                                    for (int i = 0x0000; i < 0x1000; i++)
                                    {
                                        this.PPUMemory[i + 0x1000] = this.MMC1.chrBanks[this.MMC1.highCHRBank][i];
                    ...
                    else if (this.MirrorMap[address] >= 0xA000) //Chr Reg 0
                    {
                        this.MMC1.regTmp += (byte)((value & 1) << this.MMC1.writeLatch);
                        this.MMC1.reg1 = this.MMC1.regTmp;
                        this.MMC1.regTmp = 0;
                        this.MMC1.writeLatch = 0;
                        if ((this.MMC1.reg0 & 0x10) == 0)
                        {
                            if (this.CHRRom.Length == 0)
                            {
                                for (int i = 0x0000; i < 0x1000; i++)
                                {
                                    this.MMC1.chrBanks[this.MMC1.lowCHRBank][i] = this.PPUMemory[i];
                                }
                                this.MMC1.lowCHRBank = this.MMC1.reg1 & 1;
                                for (int i = 0x0000; i < 0x1000; i++)
                                {
                                    this.PPUMemory[i] = this.MMC1.chrBanks[this.MMC1.lowCHRBank][i];
                                }
                                for (int i = 0x0000; i < 0x1000; i++)
                                {
                                    this.MMC1.chrBanks[this.MMC1.highCHRBank][i] = this.PPUMemory[i + 0x1000];
                                }
                                this.MMC1.highCHRBank = this.MMC1.lowCHRBank + 1;
                                if (this.MMC1.highCHRBank > 0xFF)
                                    this.MMC1.highCHRBank = 0;
                                for (int i = 0x0000; i < 0x1000; i++)
                                {
                                    this.PPUMemory[i + 0x1000] = this.MMC1.chrBanks[this.MMC1.highCHRBank][i];
                                }
                            ...
                            {
                                this.MMC1.lowCHRBank = this.MMC1.reg1 & 1;
                                if (this.MMC1.lowCHRBank < this.MMC1.chrBanks.Length)
                                {
                                    for (int i = 0x0000; i < 0x1000; i++)
                                    {
                                        this.PPUMemory[i] = this.MMC1.chrBanks[this.MMC1.lowCHRBank][i];
                        ...
                        else
                        {
                            if (this.CHRRom.Length == 0)
                            {
                                ...
                                this.MMC1.lowCHRBank = this.MMC1.reg1;
                                for (int i = 0x0000; i < 0x1000; i++)
                                {
                                    this.PPUMemory[i] = this.MMC1.chrBanks[this.MMC1.lowCHRBank][i];
                                }
                    ...
                    {
                        this.MMC1.regTmp += (byte)((value & 1) << this.MMC1.writeLatch);
                        this.MMC1.reg0 = this.MMC1.regTmp;
                        this.MMC1.regTmp = 0;
                        this.MMC1.writeLatch = 0;
                        for (int i = 0x2000; i < 0x3000; i++)
                        {
                            this.PPUMirrorMap[i] = (ushort)i;
                        }
                        if ((this.MMC1.reg0 & 3) == 0)
                        {
                            this.PPUMirror(0x2000, 0x2400, 0x0400, 1);
                            this.PPUMirror(0x2000, 0x2800, 0x0400, 1);
                            this.PPUMirror(0x2000, 0x2C00, 0x0400, 1);
                        }
                        else if ((this.MMC1.reg0 & 3) == 1)
                        {
                            this.PPUMirror(0x2400, 0x2000, 0x0400, 1);
                            this.PPUMirror(0x2400, 0x2800, 0x0400, 1);
                            this.PPUMirror(0x2400, 0x2C00, 0x0400, 1);
                        }
                        else if ((this.MMC1.reg0 & 3) == 2)
                        {
                            this.PPUMirror(0x2000, 0x2800, 0x0400, 1);
                            this.PPUMirror(0x2400, 0x2C00, 0x0400, 1);
                        }
                        else if ((this.MMC1.reg0 & 3) == 3)
                        {
                            this.PPUMirror(0x2000, 0x2400, 0x0400, 1);
                            this.PPUMirror(0x2800, 0x2C00, 0x0400, 1);
                        }
                ...
                //MessageBox.Show(this.Memory[0x1A + 0xC000].ToString("X2") + " PC:" + this.programCounter.ToString("X4"));
            }
            else if (this.mapper == -2)
            {
                ...
                {
                    int table;
                    if (this.PRGRom.Length <= 8)
                        table = value & 0x07;
                    else
                        table = value & 0x0F;
                    if (table < this.PRGRom.Length)
                    {
                        if (value == this.Memory[this.MirrorMap[address]])//Bus conflict
                            for (int i = 0x0000; i < 0x4000; i++)
                            {
                                this.Memory[i + 0x8000] = this.PRGRom[table][i];
                            }
                    }
                }
            }
            ...
            else if (this.mapper == 3)
            {
                if (this.MirrorMap[address] >= 0x8000)
                {
                    ...
                    table = value & 3;
                    if (value == this.Memory[this.MirrorMap[address]]) //Bus Conflict
                        if (table < this.CHRRom.Length)
                        {
                            for (int i = 0x0000; i < 0x2000; i++)
                            {
                                this.PPUMemory[i] = this.CHRRom[table][i];
                            }
                        }

                }
            }
            else if (this.mapper == 4) //MMC3
            {
                if (this.MirrorMap[address] >= 0x8000)
                {
                    if (this.MirrorMap[address] % 2 == 0)
                    {
                        if (this.MirrorMap[address] >= 0xE000) //IRQ disable
                        {
                            this.MMC3.irqEnable = false;
                        }
                        else if (this.MirrorMap[address] >= 0xC000) //IRQ Latch
                        {
                            this.MMC3.irqLatch = value;
                        }
                        else if (this.MirrorMap[address] >= 0xA000) //Mirroring
                        {
                            if ((value & 0xFE) != 0)
                            {
                                this.PPUMirror(0x2000, 0x2800, 0x0400, 1);
                        ...
                        {
                            this.MMC3.bankSelect = value;
                        }
                    }
                    else
                    {
                        if (this.MirrorMap[address] >= 0xE000) //IRQ enable
                        {
                            this.MMC3.irqEnable = true;
                        }
                        else if (this.MirrorMap[address] >= 0xC000) //IRQ reload
                        {
                            this.MMC3.irqReload = true;
                        }
                        else if (this.MirrorMap[address] >= 0xA000) //PRG RAM protect
                        {
                            if ((value & 0xBF) != 0)
                            {
                        ...
                        {
                            if ((this.MMC3.bankSelect & 0x07) == 0)
                            {
                                if ((this.MMC3.bankSelect & 0x80) == 0)
                                {/*
                                    this.MMC3.
                                    this.MMC1.lowCHRBank = this.MMC1.reg1;
                                    if (this.MMC1.lowCHRBank < this.MMC1.chrBanks.Length)
                            ...
                            else if ((this.MMC3.bankSelect & 0x07) == 01)
                            {
                                if ((this.MMC3.bankSelect & 0x80) == 0)
                                {
                                }
                            ...
                            else if ((this.MMC3.bankSelect & 0x07) == 02)
                            {
                                if ((this.MMC3.bankSelect & 0x80) == 0)
                                {
                                }
                            ...
                            else if ((this.MMC3.bankSelect & 0x07) == 03)
                            {
                                if ((this.MMC3.bankSelect & 0x80) == 0)
                                {
                                }
                            ...
                            else if ((this.MMC3.bankSelect & 0x07) == 04)
                            {
                                if ((this.MMC3.bankSelect & 0x80) == 0)
                                {
                                }
                            ...
                            else if ((this.MMC3.bankSelect & 0x07) == 05)
                            {
                                if ((this.MMC3.bankSelect & 0x80) == 0)
                                {
                                }
                            ...
                            else if ((this.MMC3.bankSelect & 0x07) == 06)
                            {
                            }
                            else if ((this.MMC3.bankSelect & 0x07) == 07)
                            {
                            }
                        }
                    }
        ...
        private byte[][,] GeneratePatternTables(int patternTablesPalette)
        {
            byte PPUCTRL = this.Memory[0x2000];
            byte PPUMASK = this.Memory[0x2001];
                ...
                for (int line = 0; line < 128; line++)
                {
                    ...
                    for (int column = 0; column < 128; column++)//For each pixel in scanline
                    {
                        ...
                        if (color == 0)
                        {
                            ...
                            if ((PPUMASK & 0x01) != 0) //If monochrome bit set, have NOT testing in game.
                                palColor &= 0xF0;
                            patternTables[patternTable][column, line] = palColor;
                        }
                        else
                        {
                            byte palColor = this.PalMemory[(patternTablesPalette * 4) + color];
                            if ((PPUMASK & 0x01) != 0) //If monochrome bit set, have NOT testing in game.
                                palColor &= 0xF0;
                            patternTables[patternTable][column, line] = palColor;
                        }
        ...
        {
            this.saveState.stateProgramCounter = this.programCounter;
            //this.saveState.stateMemory = (byte[])this.Memory.Clone();
            this.saveState.statePPUMemory = (byte[])this.PPUMemory.Clone();
            this.saveState.statePalMemory = (byte[])this.PalMemory.Clone();
            this.saveState.stateA = this.A;
            this.saveState.stateX = this.X;
            this.saveState.stateY = this.Y;
            this.saveState.stateS = this.S;
            this.saveState.stateP = this.P;
            this.saveState.stateCounter = this.counter;
            this.saveState.stateSlCounter = this.slCounter;
            this.saveState.stateScanline = this.scanline;
            this.saveState.stateVblank = this.vblank;
            this.saveState.stateSPRMemory = (byte[])this.SPRMemory.Clone();
            this.saveState.stateInterruptReset = this.interruptReset;
            this.saveState.stateInterruptIRQ = this.interruptIRQ;
            this.saveState.stateInterruptNMI = this.interruptNMI;
            this.saveState.vertOffset = this.vertOffset;
            this.saveState.horzOffset = this.horzOffset;
            this.saveState.nameTableOffset = this.nameTableOffset;
            this.saveState.loopyT = this.loopyT;
            this.saveState.loopyV = this.loopyV;
            this.saveState.loopyX = this.loopyX;
        ...
        public SaveState getState()
        {
            ...
            newState.stateMemory = (MemoryStore)this.Memory.Clone();
            newState.statePPUMemory = (byte[])this.PPUMemory.Clone();
        ...
        public void loadState(SaveState oldState)
        {
            ...
            this.Memory = (MemoryStore)oldState.stateMemory.Clone();
            this.PPUMemory = (byte[])oldState.statePPUMemory.Clone();
        ...
        {
            if (this.isStateStored)
            {
                this.programCounter = this.saveState.stateProgramCounter;
                this.Memory = (MemoryStore)this.saveState.stateMemory.Clone();
                this.PPUMemory = (byte[])this.saveState.statePPUMemory.Clone();
                this.PalMemory = (byte[])this.saveState.statePalMemory.Clone();
                this.A = this.saveState.stateA;
                this.X = this.saveState.stateX;
                this.Y = this.saveState.stateY;
                this.S = this.saveState.stateS;
                this.P = this.saveState.stateP;
                this.counter = this.saveState.stateCounter;
                this.slCounter = this.saveState.stateSlCounter;
                this.scanline = this.saveState.stateScanline;
                this.vblank = this.saveState.stateVblank;
                this.SPRMemory = (byte[])this.saveState.stateSPRMemory.Clone();
                this.interruptReset = this.saveState.stateInterruptReset;
                this.interruptIRQ = this.saveState.stateInterruptIRQ;
                this.interruptNMI = this.saveState.stateInterruptNMI;
                this.vertOffset = this.saveState.vertOffset;
                this.horzOffset = this.saveState.horzOffset;
                this.nameTableOffset = this.saveState.nameTableOffset;
                this.loopyT = this.saveState.loopyT;
                this.loopyV = this.saveState.loopyV;
                this.loopyX = this.saveState.loopyX;
            }
        }
        public void restart()
        {
        ...
        private void ApplyGameGenie()
        {
            for (int i = 0; i < this.gameGenieCodeNum; i++)
            {
                if (this.gameGenieCodes[i].code.Length == 6)
                    this.Memory[this.MirrorMap[this.gameGenieCodes[i].address + 0x8000]] = this.gameGenieCodes[i].value;
    ...
    struct SaveState
    {
        ...
        public MemoryStore stateMemory;
    ...
    struct MMC1
    {
        ...
        public byte reg2;
        public byte reg3;
        public byte writeLatch;
        public byte regTmp;
        public int lowCHRBank;
        public int highCHRBank;
        public byte[][] chrBanks;
    }
    struct MMC3
    {
        public byte chrBank1;
        public byte chrBank2;
        public byte chrBank3;
        public byte chrBank4;
        public byte bankSelect;
        public byte irqCounter;
        public byte irqLatch;
        public bool irqReload;
